...
def _parse_memory_to_bytes(value: str, default_mb: int = 256) -> int:
    """Parse a memory-limit string (e.g. "256m", "2g", "512k", "1048576") to bytes.

    Tolerant of k/m/g suffixes (case-insensitive) and bare byte counts. Upstream's
    original ``int(value.rstrip("mM")) * 1024 * 1024`` crashed on "g"-suffixed values
    like our 32g CLI-tool limit; this keeps the hardened path from throwing.
    """
    ...
    def _build_preexec_fn(self, work_path: Path, timeout: int):
        def _preexec():
            ...
            try:
                ...
                memory_bytes = _parse_memory_to_bytes(self.config.memory_limit)
    ...
    async def execute(
        ...
        **kwargs
        ...
        try:
            ...
            if not getattr(self.config, "hardened", False):
                # Relaxed default path: direct execution, no bwrap/preexec hardening.
                # Binary CLI tools depend on this — bwrap + RLIMIT_AS break Node SEA
                # binaries (need >8GB virtual memory). Real isolation for agent code
                # execution is provided by the remote aio-sandbox backend; this local
                # backend stays relaxed by design. Opt into the hardened path (bwrap +
                # preexec resource limits) by setting sandbox config `hardened: true`.
                if language == "python":
                    cmd_prefix = ["python3"]
                elif language == "bash":
                    cmd_prefix = ["bash"]
