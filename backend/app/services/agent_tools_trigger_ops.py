...
async def _handle_set_trigger(
    ...
    *,
    ...
    if name:
        from app.services.agent_tools_trigger_management import update_locked, result_json
        from app.services.trigger_patch import serialize_trigger, recover_enabled_trigger
        try:
            async with async_session() as db:
                existing = (await db.execute(select(AgentTrigger).where(
                    AgentTrigger.agent_id == agent_id, AgentTrigger.name == name,
                ).with_for_update())).scalar_one_or_none()
                if existing:
                    if existing.is_enabled:
                        return f"❌ Trigger '{name}' already exists and is active. Use update_trigger."
                    if arguments.get("type", existing.type) != existing.type:
                        return "❌ Cannot change an existing trigger type"
                    values = await update_locked(db, existing, {**arguments, "is_enabled": True}, user_id)
                    await db.commit()
                    await recover_enabled_trigger(existing, values)
                    try:
                        from app.services.audit_logger import write_audit_log
                        await write_audit_log("trigger_updated", {"name": name, "fields": sorted(values)}, agent_id=agent_id)
                    except Exception:
                        pass
                    return result_json({"ok": True, "message": f"✅ Trigger '{name}' re-enabled", "trigger": serialize_trigger(existing)})
        except Exception as exc:
            return f"❌ Failed to re-enable trigger: {exc}"
    ttype = arguments.get("type", "").strip()
    from app.services.trigger_patch import reject_reserved
    try:
        config = arguments.get("config", {})
        if not isinstance(config, dict):
            raise ValueError("Trigger config must be an object")
        config = {key: value for key, value in config.items() if not key.startswith("_")}
        reject_reserved(config)
        if "token" in config:
            raise ValueError("Webhook callback token is generated by the platform")
        config = dict(config)
    ...
    try:
        async with async_session() as db:
            ...
            _a_result = await db.execute(select(_AgentModel).where(_AgentModel.id == agent_id).with_for_update())
            ...
            if existing:
                return f"❌ Trigger '{name}' already exists. Use update_trigger."
            else:
            ...
            from app.services.trigger_patch import prepare_patch, apply_patch
            patch = {key: arguments[key] for key in (
                "reason", "max_fires", "cooldown_seconds", "expires_at", "clear_fields"
            ) if key in arguments}
            patch["config"] = {key: value for key, value in config.items() if not key.startswith("_")}
            values = prepare_patch(trigger, patch, await get_agent_timezone(agent_id))
            trigger.focus_ref = await ensure_focus_item(
                agent_id, focus_ref=focus_ref, description=reason or name, system=False, db=db,
            )
            apply_patch(trigger, values)
            # Fix 4: Safety cap for on_message triggers —
            # prevent infinite loops if agent creates broad watchers.
            if ttype == "on_message":
                if "max_fires" not in arguments and "/max_fires" not in arguments.get("clear_fields", []):
                    trigger.max_fires = 100
                if "expires_at" not in arguments and "/expires_at" not in arguments.get("clear_fields", []):
        ...
        from app.services.trigger_patch import serialize_trigger
        from app.services.agent_tools_trigger_management import result_json
        full_record = result_json({"ok": True, "message": f"✅ Trigger '{name}' created", "trigger": serialize_trigger(trigger)})
        ...
        if ttype == "webhook":
            ...
            return result_json({"ok": True, "message": message, "trigger": serialize_trigger(trigger, base_url=base)})
