...
BUILTIN_TOOLS = [
    ...
    },
    {
        ...
        "description": "Read the messages of one session you have access to (use list_sessions / search_sessions to find its id). Read-only. Returns messages in chronological order with per-message and total-size caps; pass the 'before' cursor from the previous reply to page into older messages. Access is enforced: you can only read a session you are a party to and that the current conversation partner is allowed to see; tool-call and internal rows are excluded by default.",
        "category": "discovery",
        "icon": "💬",
        "is_default": True,
        "parameters_schema": {
    ...
    },
    {
        ...
        "parameters_schema": {
            ...
            "properties": {
                ...
                "since": {"type": "string", "description": "Optional ISO8601 lower bound on activity."},
                "until": {"type": "string", "description": "Optional ISO8601 upper bound on activity."},
            },
            "required": ["query"],
        },
        "config": {},
    ...
    # --- Pages: public HTML hosting ---
    {
        ...
        "description": (
            ...
            "same URL and preserves its current permissions unless access_mode is explicitly supplied. Authorized pages "
            "render the stored HTML directly without platform-added execution restrictions. Automatic SSO is opt-in "
            "only: append auto_login=1 to the Page URL only "
