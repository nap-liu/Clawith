...
BUILTIN_TOOLS = [
...

    {
        ...
        "description": (
            ...
            "\n"
            "• Dependencies install into per-agent locations so multiple "
            "agents on the same sandbox NEVER step on each other's package "
            "versions:\n"
            "  - `pip install <pkg>` → $HOME/.local/lib/python3.10/site-packages/ "
            "(equivalent to `pip install --user`, set globally via PIP_USER=1).\n"
            "  - `npm install <pkg>` → ./node_modules (per-cwd, always isolated).\n"
            "  - `npm install -g <pkg>` → $HOME/.npm-global/ (per-agent).\n"
            "  - `apt install <pkg>` (sudo) is container-wide and shared. Use "
            "    sparingly; it also gets reset on container rebuild.\n"
            "  Pip / npm packages survive across calls and container restarts.\n"
            "  The jupyter kernel is locked to python3.10 so a `pip install` "
            "  from a shell cell is immediately importable from a python cell.\n"
            "\n"
            "• Pre-installed tools: python3.10/3.11/3.12, node v22, git, gh, "
            "uv, curl, wget, vim/nano, jq, rg, htop, imagemagick, yt-dlp, "
            "openssh client (ssh, scp, ssh-keygen, ssh-agent, rsync). "
            "requests, numpy, pandas are pre-installed in the system Python.\n"
            "\n"
            "• HOME is pinned to the agent root, so `~/.ssh/`, `~/.gitconfig`, "
            "`~/.npm/`, `pip install --user`, etc. all land in your own "
            "workspace and are isolated from other agents. Use ssh-keygen + "
            "ssh-copy-id (or write the public key to a target host another "
            "way) to set up password-less SSH, just like on a normal Linux "
            "box — no sshpass needed. The key pair persists in workspace, "
            "so subsequent calls just `ssh user@host` and it works.\n"
            "\n"
