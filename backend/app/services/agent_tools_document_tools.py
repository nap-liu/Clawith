...
def _read_document_sync(
    ...
    max_chars = min(max(int(max_chars), 1), root_agent_tools._READ_DOCUMENT_HARD_CHAR_CEILING)
    try:
        file_path = root_agent_tools._resolve_tool_source_path(ws, rel_path, tenant_id=tenant_id)
    ...
    try:
        ...
        file_size = 0
    if file_size > root_agent_tools._READ_DOCUMENT_MAX_FILE_BYTES:
        return (
    ...
    try:
        if ext == ".pdf":
            ...
            with pdfplumber.open(str(file_path)) as pdf:
                ...
                for i, page in enumerate(pdf.pages[:root_agent_tools._READ_DOCUMENT_MAX_PAGES]):
                    page_text = page.extract_text() or ""
                    if page_text:
                        text_parts.append(f"--- Page {i + 1} ---\n{page_text}")
                    if sum(len(part) for part in text_parts) >= max_chars:
                        break
                if total_pages > root_agent_tools._READ_DOCUMENT_MAX_PAGES:
                    text_parts.append(
                        f"[PDF has {total_pages} pages; only the first {root_agent_tools._READ_DOCUMENT_MAX_PAGES} were extracted]"
                    )
            ...
            def _extract_table(table) -> str:
                ...
                for row in table.rows:
                    cells = [
                        _safe_document_cell_text(cell.text).strip()
                        for cell in row.cells[:root_agent_tools._READ_DOCUMENT_MAX_COLUMNS]
                    ]
                    if not cells:
                        continue
            ...
            stop = False
            for ws_name in all_sheet_names[:root_agent_tools._READ_DOCUMENT_MAX_SHEETS]:
                if stop:
                    break
                sheet = wb[ws_name]
                ...
                for row in sheet.iter_rows(
                    max_row=root_agent_tools._READ_DOCUMENT_MAX_ROWS,
                    max_col=root_agent_tools._READ_DOCUMENT_MAX_COLUMNS,
                    values_only=True,
                ):
                    ...
                    if total_chars >= max_chars:
                        ...
                        stop = True
                        break
                if not stop and declared_rows and declared_rows > root_agent_tools._READ_DOCUMENT_MAX_ROWS:
                    rows.append(
                        f"[sheet '{ws_name}' has {declared_rows} rows; only the first "
                        f"{root_agent_tools._READ_DOCUMENT_MAX_ROWS} are shown — narrow the columns or query with sql_execute]"
                    )
                if rows:
                    sheets.append(f"=== Sheet: {ws_name} ===\n" + "\n".join(rows))
            if len(all_sheet_names) > root_agent_tools._READ_DOCUMENT_MAX_SHEETS:
                sheets.append(
                    f"[workbook has {len(all_sheet_names)} sheets; only the first {root_agent_tools._READ_DOCUMENT_MAX_SHEETS} are shown]"
                )
            wb.close()
            ...
            for i, slide in enumerate(all_slides[:root_agent_tools._READ_DOCUMENT_MAX_SLIDES]):
                texts = []
            ...
            if len(all_slides) > root_agent_tools._READ_DOCUMENT_MAX_SLIDES:
                slides.append(
                    f"[presentation has {len(all_slides)} slides; only the first {root_agent_tools._READ_DOCUMENT_MAX_SLIDES} are shown]"
                )
...
def _read_document_worker(out_queue: mp.Queue, ws_str: str, rel_path: str, max_chars: int, tenant_id: str | None) -> None:
    try:
...
def _read_pdf_fast_sync(ws: Path, rel_path: str, max_chars: int = 8000, tenant_id: str | None = None) -> str:
    ...
    try:
        file_path = _root_agent_tools()._resolve_tool_source_path(ws, rel_path, tenant_id=tenant_id)
    except ValueError as exc:
        return str(exc)
...
def _read_pdf_fast_worker(out_queue: mp.Queue, ws_str: str, rel_path: str, max_chars: int, tenant_id: str | None) -> None:
    try:
...
def _read_pdf_fast_with_timeout(ws: Path, rel_path: str, max_chars: int = 8000, tenant_id: str | None = None) -> str:
    ...
    proc = ctx.Process(target=_read_pdf_fast_worker, args=(out_queue, str(ws), rel_path, max_chars, tenant_id), daemon=True)
    proc.start()
    proc.join(root_agent_tools._READ_DOCUMENT_FALLBACK_TIMEOUT_SECONDS)
    if proc.is_alive():
        proc.terminate()
        proc.join(2)
        if proc.is_alive():
            ...
            proc.join(1)
        return (
            f"Document read timed out after {root_agent_tools._READ_DOCUMENT_TIMEOUT_SECONDS}s, "
            f"and PDF fallback also timed out after {root_agent_tools._READ_DOCUMENT_FALLBACK_TIMEOUT_SECONDS}s. "
...
def _read_document_with_timeout(ws: Path, rel_path: str, max_chars: int = 8000, tenant_id: str | None = None) -> str:
    ...
    ctx = mp.get_context("spawn")
    out_queue: mp.Queue = ctx.Queue(maxsize=1)
    proc = ctx.Process(target=_read_document_worker, args=(out_queue, str(ws), rel_path, max_chars, tenant_id), daemon=True)
    proc.start()
    proc.join(root_agent_tools._READ_DOCUMENT_TIMEOUT_SECONDS)
    if proc.is_alive():
        proc.terminate()
        proc.join(2)
        if proc.is_alive():
...
async def _convert_csv_to_xlsx(agent_id: uuid.UUID, ws: Path, arguments: dict) -> str:
    ...
    try:
        root_agent_tools = _root_agent_tools()
        src_file = root_agent_tools._resolve_tool_source_path(ws, source_path)
        tgt_file = root_agent_tools._resolve_tool_target_path(ws, target_path)
    except ValueError as exc:
        return str(exc)
