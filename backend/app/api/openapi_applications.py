"""Platform-owned lifecycle for external system applications."""
import uuid
...
router = APIRouter(prefix="/admin/openapi/applications", tags=["OpenAPI application administration"])
admin = require_role("platform_admin")
...
@router.get("")
async def list_applications(user=Depends(admin), db: AsyncSession = Depends(get_db)):
    rows = (await db.execute(select(OpenAPIApplication).order_by(OpenAPIApplication.created_at.desc()))).scalars()
    return [projection(app) for app in rows]


@router.post("", status_code=201)
async def create_application(body: ApplicationInput, response: Response,
                             user=Depends(admin), db: AsyncSession = Depends(get_db)):
    tenant = await db.get(Tenant, body.tenant_id)
    if not tenant or not tenant.is_active:
        fail("tenant_unavailable", 400)
    secret = new_secret()
    app = OpenAPIApplication(**body.model_dump(), secret_hash=digest(secret))
    db.add(app)
    await db.flush()
...
async def update_application(app_id: uuid.UUID, body: ApplicationInput,
                             user=Depends(admin), db: AsyncSession = Depends(get_db)):
    app = await load(db, app_id)
    if app.revoked_at:
        fail("application_revoked", 409)
    if body.tenant_id != app.tenant_id:
        fail("tenant_is_immutable", 400)
...
async def revoke_application(app_id: uuid.UUID, user=Depends(admin), db: AsyncSession = Depends(get_db)):
    app = await load(db, app_id)
    app.enabled = False
    app.revoked_at = now()
    app.generation += 1
...
async def application_audit(app_id: uuid.UUID, user=Depends(admin), db: AsyncSession = Depends(get_db)):
    await load(db, app_id)
