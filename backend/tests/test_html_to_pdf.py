...
async def test_convert_html_to_pdf_chrome_timeout_falls_back(
    mock_weasy_html, mock_time, mock_popen, mock_chrome_exec, platform,
):
    ...
    assert ("--no-sandbox" in args) == (platform == "linux")
    assert ("--disable-setuid-sandbox" in args) == (platform == "linux")
