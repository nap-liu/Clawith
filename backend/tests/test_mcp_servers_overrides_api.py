...
async def test_agent_owned_override_values_never_leave_rest_api(client):
    creator, _ = await _make_user("member")
    suffix = uuid.uuid4().hex[:6]
    async with async_session() as db:
        agent = Agent(name=f"Private_{suffix}", creator_id=creator.id)
        db.add(agent)
        await db.flush()
        server = MCPServer(
            ...
            installation_key=f"stdio:{uuid.uuid4().hex}",
        )
        db.add(server)
        await db.flush()
        override = MCPServerOverride(
            ...
            credential_template="creator-secret",
            command_template="secret-command",
            args_template=["--token", "secret"],
            env_template={"TOKEN": "secret"},
            system_prompt_block="secret prompt",
        )
        db.add(override)
        await db.commit()
        server_id = server.id
    ...
    response = await client.get(
        f"/api/admin/mcp-servers/{server_id}/overrides",
        headers={"Authorization": f"Bearer {admin_token}"},
    )
    assert response.status_code == 200
    item = response.json()["agent"][0]
    assert item["credential_state"] == "set"
    for field in (
        "system_prompt_block",
        "url_template",
        "headers_template",
    ...
    # Even an update response is redacted; the value remains available only to
    # the owning Agent's zero-argument inventory tool.
    updated = await client.put(
        f"/api/admin/mcp-servers/{server_id}/overrides/agent/{agent_id}",
        json={"credential_template": "rotated-secret", "env_template": {"TOKEN": "rotated"}},
        headers={"Authorization": f"Bearer {admin_token}"},
    )
    assert updated.status_code == 200
    assert updated.json()["credential_state"] == "set"
    assert updated.json()["env_template"] is None
    async with async_session() as db:
        persisted = (
            await db.execute(
        ...
        assert persisted.credential_template == "rotated-secret"
        assert persisted.env_template == {"TOKEN": "rotated"}
    ...
    assert "secret.example" not in dry_run.text
    assert "rotated-secret" not in dry_run.text
    assert "secret prompt" not in dry_run.text
