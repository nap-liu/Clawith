...
- 凭据弹窗复用共享紧凑 Modal 的 24px 留白，标题、凭据字段和右侧关闭按钮按现有规范排列。
- 新建与轮换时直接保存客户端密钥，企业管理员通过“查看凭据”再次读取；不引入加密或历史兼容流程。
  查看操作沿用租户权限与审计，应用列表不返回密钥。
- 删除表单重复说明，精简中英文文案。嵌入白名单留空允许任意有效来源，填写后按列表限制；
  链接签发与兑换使用一致规则，跳转白名单规则保持不变。
- Docker 类型检查及前端构建通过；隔离 PostgreSQL 验证新建后重复查看、轮换后查看、
  跨企业与普通成员拒绝访问、空嵌入白名单签发及兑换。复用现有场景临时验收，无新增测试文件。
- 3008 使用原数据库和源码目录完成更新，Docker Chromium 实页确认弹窗留白和简化后的创建表单。
  页面截图使用占位凭据遮盖实际值；未扩大到其他浏览器或完整回归。

## 主线紧急修复同步

- 将公司主线 `443f97a2` 合入当前功能分支，无代码冲突，保留企业 OpenAPI 接入能力。
- 带入原生对话执行进程隔离，以及 DashScope TPS/TPM 限流按 HTTP 429 有界重试的修复。
- 本次没有前端、依赖或数据库迁移变更；验证使用主线现有针对性检查，不新增测试文件。
- Docker 中进程隔离基础、流式工具与回执、后台任务、DashScope 限流场景通过。
  首轮因工具种子缺失和模拟调用的隔离模式设置不完整而失败并中断；补齐后对应范围复验。
- `test_stop_waits_for_child_commit_and_cancels_all_durable_anchors` 在合并候选中单独运行仍超时；
  用公司主线 `443f97a2` 的原始源码在同一隔离环境复测也超时（均等待 before_commit 30 秒）。
  该场景未通过，不能宣称全绿；未扩大到全量回归或另行修改主线运行机制。
- 最终合并差异检查与 800 行源文件门禁通过；本次 33 个交付源文件最长 781 行。

## 2026-09-09 主线同步与 3008 更新

- 将最新公司主线 `36195133` 合入功能分支，无冲突；包含确认恢复、关联执行 STOP、
  启动恢复等待现有执行者及读图工具继承企业配置的修复。
- Docker 隔离 PostgreSQL/Redis 中复用现有检查：启动恢复与停止执行相关 20 项、
  读图处理 9 项通过。没有新增测试文件，未运行全量回归或真实视觉模型调用。
- 本次没有前端、依赖和数据库迁移变更。3008 直接沿用原数据库与文件目录，更新源码
  提交标识并重启后端；前端继续使用与当前源码一致的现有构建。
- 合并差异和源文件行数检查通过，本次 12 个交付源文件最大 776 行。

## 2026-09-09 上下文首问与实例会话

- 扩展现有员工 access 请求的 `instance_ref`、`interaction`、`embed_origin`，
  返回可直接打开的登录链接；纯 user 请求保持旧字段。capabilities 发布支持状态。
- 复用用户委托、员工权限、普通会话和执行链；实例关联在激活后更新，首问原子入库。
  Native 使用 durable resume；OpenClaw 使用既有 Gateway 队列，没有新增聊天运行时。
- 相同业务请求重复签发与并发激活只产生一条首问；内容冲突 409，未激活十分钟过期及
  已删除会话返回 410。过期快照在 token 签发时清理，轻量防重记录继续保留。
- 通用 JSON 保留原类型；共享模型历史投影附加完整资料，H5 与 Web 使用共享折叠组件。
  没有新增问题长度、上下文大小或数据行数限制，也没有新增测试文件。
- 隔离 Docker 的真实 API 验收通过：签发不执行、并发幂等、实例隔离和恢复、140KB
  上下文、权限撤销、员工下架、会话删除、409/410、OpenClaw 队列及旧 access 合同。
- Native 首问通过真实隔离子进程调用本地 HTTP 模型 fixture，完整 JSON 进入 user 输入，
  assistant 成功落库并 completed；重复激活不再调用。既有附件相关 33 项检查通过。
- Docker TypeScript/Vite 构建通过。390px Chromium 实页验证原问题和回答、折叠/展开、
  完整 JSON、输入框、连接与刷新后仅一条首问；未验证 Safari、真机或外部模型/网关回复。
- 新迁移在隔离 PostgreSQL 完成 bootstrap、降级和升级。3008 原库先做在线备份再升级，
  直接沿用原数据库、文件目录和源码，未复制环境。真实 OAuth 与新 capabilities 通过。
- 已知共享恢复链沿用原有用户消息次数不递增行为，本轮没有另建计费机制。
- 对接合同见 `docs/openapi-integration.md`，机器可读合同见 `docs/openapi-v1.json`。
  源文件门禁和最终差异检查通过，最长交付源文件 698 行；未推送或发布生产环境。

## OpenAPI 场景查询与激活

- 新增员工有效场景查询，复用已发布配置和公开 manifest 投影；不返回草稿、停用场景、
  私有提示词或 MCP 配置。access 增加可选 `scene_key`，沿用现有 scope 与登录入口。
- 登录激活时选择普通会话场景；首问先持久选择再入库，记录实际场景版本。
  实例可切换与恢复场景，已有轮次保持原版本。无实例的显式场景使用独立会话。
- 隔离 Docker API/数据库验证通过：有效列表、敏感配置投影、员工权限、无效场景拒绝、
  所选场景首问快照、实例切换/恢复、H5 空场景参数读取偏好、旧轮次版本保持；
  同请求更换场景 409，签发后停用再兑换 404 且无首问，重复激活不覆盖后续场景选择。
- 本次没有数据库迁移、前端修改或新增测试文件；沿用原 H5 和场景执行链。
  OpenClaw 继续原网关协议，没有实现远端 Native 场景提示/工具转换。
- 对接文档与机器可读 OpenAPI 同步更新，3008 更新后通过真实 OAuth capabilities
  校验。未执行外部模型、网关回复或新一轮浏览器验证。

## 主线 MCP 修复同步与 3008 更新

- 合入公司主线 `0f8a879f`，保留 OpenAPI 上下文、实例和场景能力。
  两处后台翻译冲突保留双方键；没有业务实现冲突。
- 带入 MCP 私有安装/凭据隔离、共享目录归属和撤销检查，以及 AgentTool 查询索引。
  新增汇合迁移，将索引迁移与 OpenAPI interaction 汇到单一 head，不改历史迁移。
- 隔离 Docker 中 MCP 安装、连接、刷新撤销及场景配置 31 项、原 OAuth 合同 1 项通过。
  TypeScript 和 Vite 构建通过，没有新增测试文件或扩大到全量测试。
- 空库 bootstrap、汇合迁移往返，以及等价旧 OpenAPI schema（无新索引、旧 revision）
  到最新 head 的升级通过。3008 原库在线备份后升级，原目录与数据继续直接使用。
- 相对双方父分支的交付源文件均不超过 800 行；最大 789 行。未发布生产或推送远端。
