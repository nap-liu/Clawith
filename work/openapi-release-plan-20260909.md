...
日期：2026-09-09。结论：**主分支已合并；3008 场景自动激活已由真实消息记录及用户确认，可进入发布准备。尚未达到生产切换 GO。**

本次发布企业 OpenAPI、员工与场景发现/激活、临时登录、携带资料的首问和后续 H5 对话、系统托管 SDK、Web/H5 统一参考资料展示，以及 IM `/status` 自动场景显示修复。保持原 H5 聊天流程；不追加“新会话尚未建立”的逻辑修改，用户已确认首条消息能激活即可。
...
| 主分支 SHA | `7e38af39e541a5541972a40dcb1c88868f23c3c7`；本次 `ls-remote` 确认远端未移动 |
| 当前代码候选 | `86b90beda735eeba0bc3b0f211c066b3549ea132`，包含合并及 `/status` 修复 |
| 主分支合并 | 无冲突；新增 MCP 本地化错误断言修正及发布中断说明，未新增产品代码 |
| 候选差异范围 | `7e38af39..86b90bed`，87 文件，+6737 / -157 行（含文档、生成 JSON、既有测试；不含本次计划更新） |
| 分支关系 | 主分支独有 0 个提交；功能分支独有 17 个提交，含合并及文档提交 |
| 产品版本 | backend/frontend 均为 `1.10.3`，本轮不升级语义版本 |
| 发布 SHA / ID | 当前代码候选为 `86b90bed`；包含计划更新的正式制品 SHA 在发布准备冻结，格式 `v1.10.3-<SHA7>`；本计划不代表已经发布 |
| 3008 已部署应用 SHA | `/api/version` 确认为 `86b90beda735eeba0bc3b0f211c066b3549ea132`，`/api/health` 为 ok；前端沿用此前已验证制品，后续变化仅后端/测试/文档 |
...
| IM 场景状态 | `/status` 复用统一场景解析，正确显示自动命中场景；不新增激活状态或修改会话建立规则 |
...
| 追加 IM 修复 | 既有命令用例 27 项通过；3008 最新真实普通消息保存了场景 key、revision 15 和 automatic，用户确认激活正常 |
...
4. 首问重复打开不重复执行；STOP、明确拒绝、续答和断线恢复符合已验证行为。生产真实提问只在明确授权的验收会话执行。
   钉钉自动场景验收复用一个已绑定群：先发送普通消息建立会话，再比较 `/scene status` 与 `/status` 的场景标识；重置后尚未建立会话时显示未激活不作为缺陷。
...
验证复用既有命令检查，不新增测试文件、数据库或应用环境。原 `/scene off` 用例仍预期不含 `scene_disabled`，与当前关闭自动场景的既有行为不符；对比修改前代码同样失败后，修正了这个既有断言。

用户配合 3008 验证后，已只读核实：15:00:58 的 `/status` 与 15:01:09 的 `/scene status` 发生在重置后新会话尚未建立时；15:02:20 的首条普通消息已保存 `scene_resolved=true`、`scene_revision=15`、`activation_source=automatic`，当前统一解析也返回同一有效场景。用户明确确认“只要能够激活就可以”，因此不再改动会话建立或命令语义。未由代理发送外部消息；建立会话后的两条真实状态回包尚未另行采样，生产验收按上述顺序完成即可。
