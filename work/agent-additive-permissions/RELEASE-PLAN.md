...
> 执行状态：应用已于 2026-09-10 切换，见 [发布记录](RELEASE-RESULT.md)。
> 下文保留发布前评审草案；其中全量导出、业务数据副本和停写备份要求已被用户
> 明确改为“仅表结构演练、受影响表在线热备”，不再作为本次执行要求。
> 授权、实际源码、备份范围、迁移和观察状态以发布记录及仓库外执行证据为准。
