...
export function ProjectEventLabel({ eventType }: { eventType?: string }) {
    ...
    return <>{code
        ? t(`projectAudit.events.${code}`, { defaultValue: t('projectAudit.eventFallback') })
...
export default function ProjectEventContent({
    ...
    const translatedDetail = eventType
        ...
        : '';
    const value = String(translatedDetail || content || empty || t('projectAudit.noDetails')).trim();
    const long = Array.from(value).length > maxChars || value.split('\n').length > 6;
    ...
    return <div className={`project-event-content${className ? ` ${className}` : ''}`}>
        <MarkdownRenderer content={long ? markdownPreview(value, maxChars) : value} />
        {long && <>
            <Button
                ref={anchorRef}
                type="button"
                ...
                onMouseEnter={() => { cancelClose(); setPreviewOpen(true); }}
                onMouseLeave={scheduleClose}
                onFocus={() => { cancelClose(); setPreviewOpen(true); }}
                onBlur={scheduleClose}
                onClick={() => setPreviewOpen((open) => !open)}
            >{t('projectAudit.expandDetails')}</Button>
            {previewOpen && createPortal(
                <div
                    ...
                    onMouseLeave={scheduleClose}
                ><MarkdownRenderer content={value} /></div>,
                document.body,
            )}
        </>}
    </div>;
