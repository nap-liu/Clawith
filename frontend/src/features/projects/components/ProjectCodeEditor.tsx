...
import { DOCUMENT_THEME_CHANGE_EVENT, type ResolvedTheme } from '../../../utils/themeMode';
...
const languageByExtension: Record<string, string> = {
    bash: 'shell', c: 'c', cc: 'cpp', cpp: 'cpp', cs: 'csharp', css: 'css', csv: 'plaintext',
    dockerfile: 'dockerfile', env: 'ini', go: 'go', gql: 'graphql', graphql: 'graphql', h: 'c',
    hpp: 'cpp', htm: 'html', html: 'html', ini: 'ini', java: 'java', js: 'javascript',
    json: 'json', jsonc: 'json', jsx: 'javascript', kt: 'kotlin', less: 'less', lua: 'lua',
    md: 'markdown', mjs: 'javascript', php: 'php', properties: 'ini', py: 'python', rb: 'ruby',
    rs: 'rust', scss: 'scss', sh: 'shell', sql: 'sql', swift: 'swift', toml: 'ini',
    ts: 'typescript', tsx: 'typescript', txt: 'plaintext', vue: 'html', xml: 'xml', yaml: 'yaml', yml: 'yaml',
};
...
function toMonacoColor(value: string): string | undefined {
    ...
    if (!channels || channels.length < 3 || channels.slice(0, 3).some((channel) => !Number.isFinite(channel))) return undefined;
    const bytes = channels.slice(0, 3).map((channel) => Math.max(0, Math.min(255, Math.round(channel))));
    if (channels.length > 3 && channels[3] < 1) bytes.push(Math.max(0, Math.min(255, Math.round(channels[3] * 255))));
    ...
    return (
        <div className="project-file-workspace__monaco" aria-label={ariaLabel}>
            <Editor
                ...
                loading={<span className="project-file-workspace__editor-loading">正在载入代码编辑器…</span>}
                options={{
