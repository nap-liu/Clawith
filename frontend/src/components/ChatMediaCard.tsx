...
function readableError(error: unknown, fallback: string) {
    ...
    return detail?.message || value?.message || fallback;
}
...
export default function ChatMediaCard({
    ...
    const requestPlayback = useCallback(async (resumeAt = 0, manual = false) => {
        ...
        try {
            ...
            setError(readableError(nextError, t('toolResult.playbackError')));
            onUnavailable?.();
        } finally {
            setLoading(false);
        }
    }, [agentId, attachment.path, externalStarted, externalUrl, loading, messageId, onUnavailable, t]);

    const handleMediaError = useCallback(async () => {
        const media = mediaRef.current;
        const resumeAt = media?.currentTime || 0;
        if (externalUrl) {
            setError(t('toolResult.externalPlaybackError'));
            return;
        }
        if (!ticket || recoveryRef.current || automaticRecoveryCountRef.current >= 1) {
            setError(t('toolResult.playbackInterrupted'));
            return;
        }
        if (media?.error?.code === MediaError.MEDIA_ERR_SRC_NOT_SUPPORTED) {
            setError(t('toolResult.unsupportedFormat'));
            return;
        }
        ...
        try {
            ...
            if (status.status === 'ready') {
                setError(t('toolResult.sourceUnavailable'));
                return;
            }
        } catch {
    ...
    }, [agentId, externalUrl, requestPlayback, ticket, t]);
    ...
    return (
        <section className={`chat-media-card chat-media-card--${mode} chat-media-card--${attachment.kind}`}>
            <div className="chat-media-card__header">
                ...
                {onDownload ? (
                    <button type="button" className="chat-media-card__icon-button" onClick={onDownload} aria-label={t('toolResult.downloadMedia')}>
            ...
            {playbackUrl ? (
                ...
                >
                    ...
                    <span>{t(loading ? 'toolResult.loadingPlayback' : isVideo ? 'toolResult.playVideo' : 'toolResult.playAudio')}</span>
            ...
            {error ? (
                <div className="chat-media-card__error" role="status">
                    ...
                    <span>{error}</span>
                    <button type="button" onClick={() => void requestPlayback(mediaRef.current?.currentTime || 0, true)} aria-label={t('toolResult.reload')}>
