...
이 오픈소스 다중 에이전트 협업 플랫폼은 단일 에이전트 도구와 달리 모든 AI 에이전트에게 **영구적인 정체성**, **장기 메모리**, **독립 워크스페이스**를 부여하고, 팀으로 함께 일하고 당신과 함께 일합니다.
...
### 🏢 디지털 직원, 단순한 챗봇이 아닌
이 플랫폼의 에이전트는 **조직의 디지털 직원**입니다. 전체 조직도를 파악하고, 메시지 전송, 작업 위임, 실제 업무 관계 구축이 가능합니다 — 새 팀원이 합류하듯이.
...
> **참고:** 이 플랫폼은 로컬에서 AI 모델을 실행하지 않습니다. 모든 LLM 추론은 외부 API 제공자(OpenAI, Anthropic 등)가 처리합니다. 로컬 배포는 표준 웹 애플리케이션 + Docker 오케스트레이션입니다.
