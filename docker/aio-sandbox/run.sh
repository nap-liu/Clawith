...
DOHEOF
  ...
  # clawith patch: the sandbox HTTP API (/v1/shell, /v1/file) runs as the
  # unprivileged `gem` user, but run.sh (root) writes mcp-hub.json root-owned in a
  # root-only dir. Clawith registers stdio MCP servers at runtime by overwriting
  # this file in place via the API, so it must be writable by `gem`. Make the file
  # (not the dir) group/world-writable; the registrar uses `cat > file` (in-place),
  # never `.tmp`+`mv` (which would need dir-write `gem` lacks).
  [ -f /opt/gem/mcp-hub.json ] && chmod 666 /opt/gem/mcp-hub.json || true
...
# ----------------------
. "${RUN_SCRIPT_DIR}/run_gost_proxy.sh"
