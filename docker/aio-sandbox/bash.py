...
class BashSession:
    ...
    PROCESS_SIGNAL_GRACE_SECONDS = 0.25
    ...
    def _handle_hard_timeout_command(
        ...
        try:
            ...
            # A shell builtin can install a trap that ignores Ctrl-C. There is
            # no smaller kill target than the shell process in that case. Reset
            # this BashSession in place so the API session remains usable and
            # can never enter the -1/empty-output timeout cascade. This is the
            # last-resort path and intentionally reaps that shell's background
            # jobs; ordinary external commands and interruptible builtins keep
            # the original stateful shell.
            logger.error(
                'Resetting uninterruptible shell after hard timeout: %s', exc
            )
            reset_work_dir = getattr(self, '_cwd', self.work_dir)
            self.close()
            self.work_dir = reset_work_dir
            self._initialized = False
            self.initialize()
            session_reset = True

        self.prev_status = BashCommandStatus.HARD_TIMEOUT
        if not session_reset and len(ps1_matches) != 1:
            logger.warning(
                'Expected exactly one PS1 metadata block BEFORE the execution of a command, '
                f'but got {len(ps1_matches)} PS1 metadata blocks:\n---\n{pane_content!r}\n---'
            )
        ...
        if session_reset:
            metadata.suffix += (
                '\n[The shell ignored interruption and was reset to guarantee '
                'termination; shell variables and background jobs were cleared.]'
            )
        command_output = self._get_command_output(
            command,
            raw_command_output,
