...
- Strict cross-domain rerun (2026-08-24): every domain project reached a terminal runtime state, but the public acceptance runner still rejects marketing, data, and compliance. The remaining debt is concrete rather than a generic test failure: historical Runs without work-item lineage, work items without a successful related Run, unrecovered historical failures, incomplete role-specific contribution evidence, mechanical/status-only collaboration, weak dependency topology, and milestone records without both Run and work-item links. These findings keep the cross-domain completion checkboxes open until the projects are remediated or rerun from a clean state.
