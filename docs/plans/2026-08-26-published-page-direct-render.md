...
- Backend full suite: the exact final SHA passed 2,401 tests with 28 skipped in one isolated-Docker run. No MCP code was changed.
...
- Direct-render browser: under the user's explicit instruction to use Browser directly instead of spending time building a Docker Chromium/font layer, the exact final candidate stack was opened through local port 3008 in local Chrome. This user-directed exception overrides the repository's Docker-only browser default for this final repetition. It passed top-level rendering with a non-null origin, same-origin SDK/CSS/API access, localStorage, ordinary cookies, dynamic import, Worker, fonts, forms, downloads, media support, navigation, report CSS, and explicit watermarking; page self-check and browser console errors were both empty. Earlier same-scope Docker Chromium evidence also covered cross-origin iframe rendering, user-gesture download/audio/popup, and the complete SDK OAuth flow. The pre-existing 3008 frontend container was restored and returned HTTP 200 after the isolated candidate stack was removed.
